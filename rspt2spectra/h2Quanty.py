...
def print_QuantyH_from_dense_rsptH(
    h_rspt, ang=2, previous_orbitals=6, next_orbitals=0, filename="h0.lua", op_name="H0"
):
    """
...
def write_quanty_opp(
    ...
    s += "},\n"
    s_onerow = "{"
    s_onerow += ", ".join([(str(el.real) + "+I*" + str(el.imag)) for el in e])
    s_onerow += "})"
    s += s_onerow
    s += "\n"
    if len(s_onerow) > 3000:
        print("Warning: Lua does not support " "lines longer than 3000")
...
def write_quanty_opps(
    ...
    for k, (ii, jj, ee) in enumerate(d):
        ...
        s += "},\n"
        s_onerow = "{"
        s_onerow += ", ".join([(str(el.real) + "+I*" + str(el.imag)) for el in ee])
        s_onerow += "})"
        s += s_onerow
        s += "\n"
        if len(s_onerow) > 3000:
