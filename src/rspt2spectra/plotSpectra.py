...
def fermi_smearing(x, y, fwhm):
    ...
    """
    # Inverse temperature
    beta = 4.0 * np.log(1 + np.sqrt(2)) / fwhm
...
def gaussian_smearing(x, y, fwhm):
    ...
    std = 1.0 / (2 * np.sqrt(2 * np.log(2))) * fwhm
    # grid spacing (assume uniform grid)
    dx = x[1] - x[0]
    # Grid spacing should be smaller than the
    # standard deviation
    ...
    g = 1.0 / np.sqrt(2 * pi * std**2) * np.exp(-(xg**2) / (2 * std**2))
    # This check ensures the return array has the
    # same shape as x
    assert len(x) > len(xg)
...
def get_normalization(x, minus_intensity, loc, h, peak=0, peakorder="topdown"):
    y = -minus_intensity
...
def find_peak_index(x, y, peak_nbr=0, peakorder="topdown"):
    ...
    if peakorder == "topdown" and peak_nbr == 0:
        ...
        mask1 = np.logical_and(0 <= np.diff(y[:-1]), np.diff(y[1:]) < 0)
        mask2 = np.logical_and(0 < np.diff(y[:-1]), np.diff(y[1:]) <= 0)
        ...
        if peakorder == "topdown":
            ...
            sys.exit("Warning: Value of peakorder variable" "is incorrect.")
...
def get_Quanty_spectrum(
    folder=".",
    xas_file="XASSpec.dat",
