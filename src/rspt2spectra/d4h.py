...
def get_D4h_splitting(w, pdos_x2y2, pdos_z2, pdos_xy, pdos_xz, wmin, wmax):
    """
    ...
    e_mean = (x2y2 + z2 + xy + 2 * xz) / 5.0
    deltao = 1 / 2.0 * x2y2 + 1 / 2.0 * z2 - 1 / 3.0 * xy - 2 / 3.0 * xz
    delta1 = x2y2 - z2
    delta2 = xy - xz
...
def get_delta_o(e):
    ...
    if len(e) == 2:
        ...
        xz = np.array(e[2])
        xy = np.array(e[3])
        deltao = (z2 + x2y2) / 2.0 - (2 * xz + xy) / 3.0
    else:
...
def get_3d_energy(e):
    ...
    if len(e) == 2:
        ...
        xz = np.array(e[2])
        xy = np.array(e[3])
        em = 1 / 5.0 * (1 * z2 + 1 * x2y2 + 2 * xz + 1 * xy)
    else:
        sys.exit("Wrong size of e")
    return em
...
e["o"] = 1 / 10.0 * e["q"]
e["1"] = 1 / 20.0 * e["q"] + 1 / 7.0 * e["s"] + 3 / 35.0 * e["t"]
e["2"] = -1 / 15.0 * e["q"] + 1 / 7.0 * e["s"] - 4 / 35.0 * e["t"]
...
def get_Dq_Ds_Dt(deltao, delta1, delta2):
    ...
    """
    dq = 1 / 10.0 * deltao + 1 / 60.0 * (3 * delta1 - 4 * delta2)
    ds = (delta1 + delta2) / 7.0
    dt = (3 * delta1 - 4 * delta2) / 35.0
    return dq, ds, dt
...
def get_CF_hamiltonian_with_delta(e_mean, deltao, delta1, delta2):
    ...
    Returns crystal-field hamiltonian in spherical harmonics basis.
    """
    h = e_mean * np.eye(5) + deltao * e["o"] + delta1 * e["1"] + delta2 * e["2"]
    return h
